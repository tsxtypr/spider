...
def get_base_url(url,headers):
    ...
    for href in href_list:
        get_detail_content(href)
        return

#获取详情页的text
def get_detail_content(href):
    hrefs=href.split('#')
    href1=hrefs[1]
    href2=hrefs[2]
    for i in range(1,10):
        ...
        # print(html)
        get_detail_infos(html)

#解析详情页
def get_detail_infos(html):
    html_element=etree.HTML(html)
    li_list=html_element.xpath('//ul[@class="carlist clearfix js-top"]/li')
    info_list=[]
    for li in li_list:
        item={}
        title=get_text(li.xpath('.//h2[@class="t"]/text()'))
        # print(title)
        price1=get_text(li.xpath('.//div[@class="t-price"]/p/text()'))
        price2=get_text(li.xpath('.//div[@class="t-price"]/p/span/text()'))
        price=price1+price2
        year=get_text(li.xpath('.//div[@class="t-i"]/text()'))
        kilometre=li.xpath('.//div[@class="t-i"]/text()')[1]
        # print(kilometre)
        img_url=get_text(li.xpath('.//img/@src'))
        detail_url=get_text(li.xpath('.//a/@href'))
        item['title']=title
        item['price']=price
        item['year']=year
